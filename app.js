class TinnieaseApp {
    ...
    createAudioControls() {
        ...
        const frequencyGroup = this.createControlGroup('frequency', 'Frequency (Hz):', 'range', {
            min: '20',
            max: '20000',
            value: '1000'
        });
        ...
        const waveTypeGroup = this.createControlGroup('wave-type', 'Wave Type:', 'select', {
            options: [
        ...
        const phaseGroup = this.createControlGroup('phase', 'Phase (degrees):', 'range', {
            min: '0',
            max: '360',
            value: '0'
        });
    ...
    async startConversation() {
        this.addBotMessage("Welcome to Tinniease! Are you a new or existing user? (Type 'new' or 'existing')");
    }
    ...
    async handleNewUserFlow(message) {
        if (!this.userData.name) {
            ...
            this.addBotMessage("Nice to meet you, " + message + "! What's your date of birth? (DD/MM/YYYY)");
            ...
            this.addBotMessage("What type of tinnitus do you have?\n1. Bilateral Symmetric\n2. Bilateral Asymmetric\n3. Unilateral");
    ...
    async handleExistingUserFlow(message) {
        if (!this.userData.phoneNumber) {
            this.userData.phoneNumber = message;
            this.addBotMessage("Welcome back! What type of tinnitus do you have?\n1. Bilateral Symmetric\n2. Bilateral Asymmetric\n3. Unilateral");
        } else if (!this.userData.tinnitusType) {
            const type = parseInt(message);
            if (type >= 1 && type <= 3) {
                this.userData.tinnitusType = type;
    ...
    loadTinnitusInterface(type) {
        ...
        switch(type) {
            case 1: // Bilateral Symmetric
                htmlFile = 'New folder/mono.html';
                break;
            case 2: // Bilateral Asymmetric
                htmlFile = 'New folder/binaural.html';
                break;
            case 3: // Unilateral
                htmlFile = 'New folder/uni.html';
                break;
        }
        ...
        fetch(htmlFile)
            ...
            .catch(error => {
                ...
                this.addBotMessage("Sorry, there was an error loading the interface. Please try again.");
            });
    }
    ...
    initializeTinnitusInterface(type) {
        ...
        switch(type) {
            case 1:
                this.addBotMessage("I've loaded the bilateral symmetric tinnitus interface. You can now use the controls to manage your symptoms.");
                break;
            case 2:
                this.addBotMessage("I've loaded the bilateral asymmetric tinnitus interface. You can now use the controls to manage your symptoms.");
                break;
            case 3:
                this.addBotMessage(`I've loaded the unilateral tinnitus interface for your ${this.userData.affectedEar} ear. You can now use the controls to manage your symptoms.`);
                break;
        }
    }
    ...
    showAudioControls() {
        if (this.userData.tinnitusType === 1) {
            this.addBotMessage("I've set up the audio controls for bilateral symmetric tinnitus. You can adjust the frequency, wave type, and phase difference.");
        } else if (this.userData.tinnitusType === 2) {
            this.addBotMessage("I've set up the audio controls for bilateral asymmetric tinnitus. You can adjust the frequency and wave type for each ear independently.");
        } else {
            this.addBotMessage(`I've set up the audio controls for unilateral tinnitus in your ${this.userData.affectedEar} ear. You can adjust the frequency, wave type, and phase.`);
        }
    }
